...
% load true symbols
payloadSF8 = load("Input/singlePacket/125e3_8_0cf_gth_112sym.mat").grd_truth_SF8;
...
[outputSig,~] = LGIC_v2(rxSig,localSigSF8,fs,bw,8,payloadSF8);
...
if (SHOW_SIGNALS > 0)
    ...
    subplot(212);
    plot(abs(outputSig),LineWidth=0.1,color=[0.2 0.2 0.2],Markersize=2,MarkerFaceColor=[0.2 0.2 0.2]);
    ylim([0 0.03]);
    xlabel('Samples');
    ylabel('Amplitude');
...
if (SHOW_SPECTRUM > 0)
    ...
    subplot(212);
    pspectrum(outputSig,fs,'spectrogram','OverlapPercent',99,'Leakage',0.85,'MinThreshold',-70,'TimeResolution',0.001);
    xlabel('Time (ms)');
    ylabel('Freq (kHz)');
